...
async function main() {
  ...
  // Obtener la cuenta para firmar (primer cuenta Hardhat)
  ...
  try {
    // Prueba llamar obtenerPresidente()
    if (contract.obtenerPresidente) {
      const presidente = await contract.obtenerPresidente();
      console.log("Presidente:", presidente);
    } else {
      console.log("No existe función obtenerPresidente()");
    }

    // Prueba llamar rondaActual()
    const ronda = await contract.rondaActual();
    console.log("Ronda actual:", ronda);

  } catch (error) {
    console.error("Error al llamar funciones:", error);
  }
}
